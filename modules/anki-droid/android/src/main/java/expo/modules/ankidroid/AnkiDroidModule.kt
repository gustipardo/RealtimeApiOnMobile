...
class AnkiDroidModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    // Modern AnkiDroid (2.16+) auto-prompts when ContentProvider is accessed
    // We just open AnkiDroid to ensure it's initialized
    AsyncFunction("requestApiPermission") { promise: Promise ->
      try {
        ...
        if (launchIntent != null) {
          ...
          promise.reject("NOT_FOUND", "AnkiDroid app not found", null)
        }
      } catch (e: Exception) {
        promise.reject("REQUEST_FAILED", "Failed to open AnkiDroid: ${e.message}", e)
      }
    }
